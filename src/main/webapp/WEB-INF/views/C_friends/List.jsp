    ...
    <script type="text/javascript">
    	$(document).ready(function() {
	    	...
	    	$(document).on("click", ".title_btn", function() {
				...
				location.href="${contextPath}/friendsBoard/friendsBoardView?num="+num+"&nowpage=${map.nowpage}&sort="+sort+"&search=${map.search}";
			});
    		
	    	//검색 기능
	    	$(".sbutton").click(function() {
	    		var search = $(".search").val();
	    		$("#sform").attr("action", "${contextPath}/friendsBoard/friendsBoardList");
				$("#sform").submit();
			});
	    	...
	    	function paging(data) {
	    		...
	    		if(data.blockfirst != 1) {
	    			pstr += "<a href='${contextPath}/friendsBoard/friendsBoardList?nowpage="+data.blockfirst-1+"&search="+data.search+"'"+"><i class='fa fa-angle-left page' data-num='"+(data.blockfirst-1)+"'></i></a>";
	    		}
	    		for(var i= data.blockfirst; i<=data.blocklast; i++) {
	    			if(data.nowpage == i) {
	    				pstr += "<a class='active page pagenum' href='${contextPath}/friendsBoard/friendsBoardList?nowpage="+i+"&search="+data.search+"&sort="+data.sort+"'"+" style='background: #cdebfa;' data-now='"+data.nowpage+"' data-num='"+i+"'>"+i+"</a> "; 
	    			} else {
				...
				$.ajax({
					...
					success : function(data) {
						...
						$.each(data.list, function(i) {
							...
							var membercount = data.list[i].membercount;
							var nowcount = data.list[i].nowcount;
            ...
            <div class="row">
              <div class="col-sm-12">
                <table class="table table-striped table-border checkout-table">
                    ...
                    <tbody id="Ltbody">
                    <c:forEach var="list" items="${map.friendsBoardList}">
              ...
              <div class="col-sm-12" style="text-align: center;">
                ...
                <a href="${contextPath}/friendsBoard/friendsBoardList?nowpage=${map.blockfirst-1}&search=${map.search}&sort=${map.sort}"><i class="fa fa-angle-left page" data-num="${map.blockfirst-1}"></i></a>
                </c:if>
                <c:forEach begin="${map.blockfirst}" end="${map.blocklast}" var="i">
                <c:if test="${map.nowpage == i }">
                <a class="active page pagenum" style="background: #cdebfa;" href="${contextPath}/friendsBoard/friendsBoardList?nowpage=${i}&search=${map.search}&sort=${map.sort}" data-now="${map.nowpage}" data-num="${i}">${i}</a>
                </c:if>
                <c:if test="${map.nowpage != i }">
                <a class="active page pagenum" href="${contextPath}/friendsBoard/friendsBoardList?nowpage=${i}&search=${map.search}&sort=${map.sort}" data-now="${map.nowpage}" data-num="${i}">${i}</a>
                </c:if>
                </c:forEach>
                <c:if test="${map.blocklast != map.totalpage}">
                <a href="${contextPath}/friendsBoard/friendsBoardList?nowpage=${map.blocklast+1}&search=${map.search}&sort=${map.sort}"><i class="fa fa-angle-right page" data-num="${map.blocklast+1}"></i></a>
                </c:if>
                </div>
              </div>
             <!-- 페이징 -->
